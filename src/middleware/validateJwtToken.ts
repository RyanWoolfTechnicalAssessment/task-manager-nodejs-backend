...
const validateJwtTokenUser = async (req: Request, res:Response, next: () => void) => {
    ...
    const unsecuredPathList:string[]=["/user/login"];
    ...
    if ((!req.headers.authorization || !req.headers.authorization.startsWith("Bearer ")) &&
        ...
        res.status(401).json({success:false,status: 401,error:"Unauthorized"});
        return;
    }
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith("Bearer ")) {
        ...
        // No cookie
        res.status(403).json({success:false,status: 403,error:"Unauthorized"});
        return;
    }
    ...
    try {
        ...
        if(verifyTokenResponse.success){
            ...
            logger.log("ID Token correctly decoded", verifyTokenResponse.data?.userName);
            ...
            await db.user.findOne({where: {
                        ...
                        await db.userrole.findAll({
                            ...
                            if (roleList.length>0) {
                                for (const tempRole in roleList) {
                                    ...
                                    // @ts-ignore
                                    if (endpointRoleMap.get(path).includes(roleList[tempRole])) {
                                        console.log("7");
                                        rolePermitted=true;
                                        next();
                                        return;
                                ...
                                if (!rolePermitted) {
                                    logger.error("Roles Assign to third party not permitted to access endpoint");
                                    res.status(401).json({success:false,status: 401,error:"Unauthorized"});
                                }
                            } else {
                                logger.error("Error third party does not have roles assigned");
                                res.status(401).json({success:false,status: 401,error:"Unauthorized"});
                            }
                        }).catch((error:Error)=>{
                            logger.error("Error getting member roles:"+error.message);
                            res.status(500).json({success:false,status: 403,error:"Error getting third party roles"});
                        });
                ...
                logger.error("Error Auth Third Party in local database:"+error.message);
                res.status(401).json({success:false,status: 401,error:"Unauthorized"});
                return;
            });
        ...
        res.status(500).json({success:false,status: 500,error:"Internal Server error while verifying token"});
        return;
    }
};
