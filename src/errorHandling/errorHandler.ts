...
export const errorHandler = async (error: Error, req: Request, res:Response, next: () => void) => {

   if (error instanceof ErrorResponseHandler)
    {

        if(error.sendSupportMail){
            await sendSupportEmail(error.message, error.errorCode, error.originalError);
        }
