...
export class AddUserRoleImpl implements IAddUserRole {
    ...
    userRole: any;
    userRoleStatus: any;
    constructor(addUserRoleRequest: IAddUserRoleRequest | undefined, userRepository:IUserRepository) {
    ...
    async checkIfRoleExists(): Promise<void> {
        ...
        if(this.addUserRoleRequest&&this.addUserRoleRequest.authority){
            this.role = await this.userRepository.findRoleByAuthority(this.addUserRoleRequest?.authority);
        }
    ...
    async checkIfUserExists(): Promise<void> {
        if(this.addUserRoleRequest&&this.addUserRoleRequest.userName){
            this.user = await this.userRepository.findUserByUserName(this.addUserRoleRequest?.userName)
        }
    }

    async getActiveUserRoleStatus(): Promise<void> {
        this.userRoleStatus = await this.userRepository.findUserRoleStatusByStatusCode("ACTIVE")
    }
    ...
    async createUserRole(): Promise<void> {
        if(this.user!=null&&this.role!=null&&this.userRoleStatus!=null){
            ...
            this.userRole = await this.userRepository.createUserRole(userRoleInputAttributes);
        }
